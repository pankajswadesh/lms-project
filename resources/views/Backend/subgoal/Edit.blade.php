{{--<form id="validation2" action="{{ route('updatesubGoal', $records->id) }}" class="form-horizontal" method="post" enctype="multipart/form-data" data-subgoal-id="{{ $records->id }}">--}}
{{--    {{csrf_field()}}--}}
...
{{--        $.ajax({--}}
{{--            url: '{{ route("deletesubGoal", ["id" => ":subgoalId"]) }}'.replace(':subgoalId', subgoalId),--}}
{{--                    type: 'GET',--}}
{{--                    headers: {--}}
{{--                        'X-CSRF-TOKEN': '{{ csrf_token() }}'--}}
{{--                    },--}}
{{--                    success: function(response) {--}}
{{--                        toastr.success('Goal and its children deleted successfully!');--}}

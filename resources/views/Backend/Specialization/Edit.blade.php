<form id="validation2" action="{{route('updateSpecialization',$records->id)}}" class="form-horizontal" method="post" enctype="multipart/form-data">
    {{csrf_field()}}
        ...
        <div class="form-group">
            ...
            <div class="col-sm-8">
                <select name="status"  class="validate[required] select2" data-placeholder="Select Status">
                    ...
                    <option value="Active" {{ $records->status == 'Active' ? 'selected' : '' }}>Active</option>
                    <option value="Inactive"  {{ $records->status == 'Inactive' ? 'selected' : '' }} >Inactive</option>
